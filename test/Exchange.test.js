...
contract('Exchange', ([deployer, feeAccount, user1, user2, user3]) => {
	...
	describe('making orders', async () => {
		...
		it('emits an Order event', async() => {
			const event = eventCheck(result, 'Order')
		...
		describe('filling orders', async () => {
			...
			describe('success', async () => {
				...
				it('executes the trade and charges fees', async () => {
					...
					balance.toString().should.equal(ether(1).toString(), 'user2 received Ehter')
				...
				it('emits a Trade event', async () => {
					const event = eventCheck(result, 'Trade')
					event.id.toString().should.equal('1', 'id is correct')
					event.user.should.equal(user1, 'user is correct')
					event.tokenGet.should.equal(token.address, 'tokenGet is correct')
					event.amountGet.toString().should.equal(tokens(1).toString(), 'amountGet is correct')
			...
			describe('failure', async () => {
				it('rejects in valid order ids', async() => {
		...
		describe('cancelling orders', async () => {
			...
			describe('success', async () => {
				...
				it('emits a Cancel event', async() => {
					const event = eventCheck(result, 'Cancel')
					event.id.toString().should.equal('1', 'id is correct')
					event.user.should.equal(user1, 'user is correct')
					event.tokenGet.should.equal(token.address, 'tokenGet is correct')
					event.amountGet.toString().should.equal(tokens(1).toString(), 'amountGet is correct')
